...
npiList = unique(asrs2$npi)
asrs2 = filter(cmsx, npi %in% npiList)
...
codes1 = asrs1 %>% group_by(code, description) %>% summarise(count = n()) %>%  arrange(-desc(count))

codes2 = asrs2 %>% group_by(code, description) %>% summarise(count = n()) %>%  arrange(-desc(count))
